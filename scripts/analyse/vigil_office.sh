#!/bin/bash
set -uo pipefail

# --- Analyse des fichiers bureautiques ---
#
# Inspire de scalpel/bin/offices : extraction des fichiers bureautiques
# depuis les peripheriques montes dans /investigation/.
#
# Le script :
#   1. detecte tous les fichiers bureautiques dans /investigation/ (par
#      extension + verification du type MIME via `file`) ;
#   2. les classe par categorie (Document, Tableur, Presentation, PDF, RTF)
#      et par format (extension) pour les statistiques ;
#   3. calcule la signature SHA-256 de chaque fichier ;
#   4. genere un rapport PDF (dans /opt/vigil/rapports).
#
# La detection par extension est necessaire car `file` ne distingue pas les
# formats OOXML (docx/xlsx/pptx sont tous renvoyes comme application/zip).
# On verifie ensuite le type MIME reel pour confirmer la nature du fichier.
#
# Les fichiers de travail (liste, JSON) sont stockes dans un dossier temporaire
# nettoye a la fin. Les copies optionnelles des fichiers se font vers le dossier
# choisi par l'utilisateur (option --copy-to).

# --- Couleurs ---
RED='\e[91m'
GREEN='\e[92m'
YELLOW='\e[93m'
BLUE='\e[96m'
GREY='\e[90m'
BOLD='\e[1m'
NC='\e[0m'

print_banner() {
    echo -e "${BLUE}"
    cat <<'VIGILART'
█   █ ███  ███  ███ █     
█   █  █  █      █  █     
█   █  █  █  ██  █  █     
 █ █   █  █   █  █  █     
  █   ███  ███  ███ █████ 
VIGILART
    echo -e "${NC}"
}

ERRORS=0

VIGIL_BASE="${VIGIL_BASE:-/opt/vigil}"
VIGIL_DATA_DIR="${VIGIL_DATA_DIR:-$VIGIL_BASE/data}"
export VIGIL_BASE VIGIL_DATA_DIR
ACTIVE_PROJECT_FILE="$VIGIL_BASE/data/active_project"

# --- Extensions bureautiques cibles et leur categorie ---
# Format : "ext:category:label"
OFFICE_SPECS=(
    "doc:document:Word 97-2003 (.doc)"
    "docx:document:Word OOXML (.docx)"
    "dotx:document:Modèle Word (.dotx)"
    "docm:document:Word Macro (.docm)"
    "odt:document:OpenDocument Text (.odt)"
    "ott:document:Modèle OpenDocument (.ott)"
    "rtf:document:Rich Text Format (.rtf)"
    "xls:spreadsheet:Excel 97-2003 (.xls)"
    "xlsx:spreadsheet:Excel OOXML (.xlsx)"
    "xltx:spreadsheet:Modèle Excel (.xltx)"
    "xlsm:spreadsheet:Excel Macro (.xlsm)"
    "ods:spreadsheet:OpenDocument Spreadsheet (.ods)"
    "ots:spreadsheet:Modèle OpenDocument Calc (.ots)"
    "csv:spreadsheet:CSV (.csv)"
    "ppt:presentation:PowerPoint 97-2003 (.ppt)"
    "pptx:presentation:PowerPoint OOXML (.pptx)"
    "potx:presentation:Modèle PowerPoint (.potx)"
    "ppsx:presentation:PowerPoint Show (.ppsx)"
    "odp:presentation:OpenDocument Presentation (.odp)"
    "otp:presentation:Modèle OpenDocument Impress (.otp)"
    "pdf:pdf:PDF (.pdf)"
)

# --- Parsing des arguments ---
# --pdf          : forcer la generation du rapport PDF (comportement par defaut)
# --no-pdf       : desactiver la generation du rapport PDF
# --copy-to DIR  : copier les fichiers bureautiques vers DIR
GEN_PDF=1
COPY_TO=""
JSON_OUT=""
while [ $# -gt 0 ]; do
    case "$1" in
        --pdf)              GEN_PDF=1 ;;
        --no-pdf)           GEN_PDF=0 ;;
        --copy-to)          shift; COPY_TO="${1:-}" ;;
        --copy-to=*)        COPY_TO="${1#--copy-to=}" ;;
        --json-out)         shift; JSON_OUT="${1:-}" ;;
        --json-out=*)       JSON_OUT="${1#--json-out=}" ;;
    esac
    shift
done
ACTIVE_USER_FILE="$VIGIL_BASE/data/active_user"
PROJECTS_DIR="$VIGIL_BASE/data/projects"
CONFIG_FILE="$VIGIL_BASE/data/config/system.json"
INVESTIGATION_DIR="${VIGIL_INVESTIGATION_DIR:-/investigation}"
STAMP=$(date +"%Y%m%d_%H%M%S")
WORK_DIR=$(mktemp -d -t "vigil_office_${STAMP}.XXXXXX")
RAPPORTS_DIR="$VIGIL_BASE/rapports"

# --- Verifier qu'un utilisateur (obligatoire) est actif ---
if [ ! -f "$ACTIVE_USER_FILE" ]; then
    echo -e "${RED}❌ Aucun utilisateur actif sélectionné.${NC}"
    echo "Sélectionnez un utilisateur via l'interface GUI."
    exit 1
fi
ACTIVE_USER=$(cat "$ACTIVE_USER_FILE")
ACTIVE_PROJECT=$(cat "$ACTIVE_PROJECT_FILE" 2>/dev/null || echo "")
ACTIVE_ENTITY="(non configurée)"
if [ -f "$CONFIG_FILE" ] && command -v jq >/dev/null 2>&1; then
    ACTIVE_ENTITY=$(jq -r '.entity_name // empty' "$CONFIG_FILE" 2>/dev/null || echo "")
    [ -z "$ACTIVE_ENTITY" ] && ACTIVE_ENTITY="(non configurée)"
fi

# --- final_pause : ne ferme jamais le terminal sans lecture ---
final_pause() {
    echo ""
    if [ "$ERRORS" -gt 0 ]; then
        echo -e "${RED}❌ Analyse terminée avec $ERRORS erreur(s).${NC}"
        echo -e "${YELLOW}Appuyez sur Entrée pour fermer ce terminal...${NC}"
        read -r
        exit 1
    else
        echo -e "${GREEN}✅ Analyse terminée sans erreur.${NC}"
    fi
}

fail() {
    echo -e "${RED}❌ $1${NC}"
    log_action "error" "$1"
    ERRORS=$((ERRORS + 1))
    final_pause
    exit 1
}

log_action() {
    local timestamp
    timestamp=$(date +"%Y-%m-%dT%H:%M:%S.%6NZ")
    local log_entry="[$timestamp] User:$ACTIVE_USER | Entity:$ACTIVE_ENTITY | Project:$ACTIVE_PROJECT | Action:office_analysis | Status:$1 | Message:$2"
    echo "$log_entry" | tee -a "$PROJECT_LOG_DIR/chain_of_custody.log" > /dev/null 2>&1 || true
}

# --- Construire les tables de correspondance extension -> categorie/label ---
declare -A EXT_CATEGORY
declare -A EXT_LABEL
for spec in "${OFFICE_SPECS[@]}"; do
    ext="${spec%%:*}"
    rest="${spec#*:}"
    cat="${rest%%:*}"
    label="${rest#*:}"
    EXT_CATEGORY["$ext"]="$cat"
    EXT_LABEL["$ext"]="$label"
done

# --- Projet obligatoire (chaine de custody) ---
if [ -z "$ACTIVE_PROJECT" ] || [ "$ACTIVE_PROJECT" = "(no-project)" ]; then
    echo -e "${RED}❌ Un projet actif est obligatoire pour cette analyse "
    echo -e "${RED}(chaîne de custody). Sélectionnez un projet via la GUI.${NC}"
    exit 1
fi
PROJECT_LOG_DIR="$PROJECTS_DIR/$ACTIVE_PROJECT"
if [ ! -d "$PROJECT_LOG_DIR" ] || [ ! -w "$PROJECT_LOG_DIR" ]; then
    echo -e "${RED}❌ Le dossier du projet est introuvable ou non inscriptible : $PROJECT_LOG_DIR${NC}"
    exit 1
fi

# --- Verification des dependances ---
echo -e "${BLUE}=== Vérification des dépendances ===${NC}"
MISSING=0
for cmd in file find stat; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
        echo -e "${RED}❌ Outil requis absent : $cmd${NC}"
        MISSING=$((MISSING + 1))
    fi
done
[ "$MISSING" -gt 0 ] && fail "Dépendances obligatoires manquantes ($MISSING)."

clear
print_banner

echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  Vigil - Analyse des fichiers bureautiques ${NC}"
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}Projet : ${ACTIVE_PROJECT}${NC}"
log_action "start" "Début de l\'analyse"
echo -e "${BLUE}Entité : ${ACTIVE_ENTITY} | Utilisateur : ${ACTIVE_USER}${NC}"
echo ""

# --- Verifier que /investigation contient des peripheriques montes ---
echo -e "${BLUE}=== Vérification des périphériques montés ===${NC}"
MOUNTED_DIRS=0
while IFS= read -r -d '' dir; do
    if mountpoint -q "$dir" 2>/dev/null; then
        MOUNTED_DIRS=$((MOUNTED_DIRS + 1))
    fi
done < <(find "$INVESTIGATION_DIR" -maxdepth 1 -mindepth 1 -type d -print0 2>/dev/null)

if [ "$MOUNTED_DIRS" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Aucun périphérique monté dans /investigation/.${NC}"
    echo -e "${YELLOW}    Montez d'abord un périphérique via le menu \"Monter\".${NC}"
    fail "Aucun périphérique à analyser."
fi
echo -e "${GREEN}✅ ${MOUNTED_DIRS} périphérique(s) monté(s) dans /investigation/.${NC}"
echo ""

# --- Detection des fichiers bureautiques ---
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  Étape 1 : Détection des fichiers bureautiques${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}Recherche des fichiers bureautiques dans /investigation/...${NC}"
echo ""

# Liste des fichiers bureautiques : detection par extension + MIME
# Format TSV : chemin_absolu<TAB>mime<TAB>taille<TAB>date_modif_iso<TAB>chemin_partition<TAB>ext<TAB>categorie
OFFICE_LIST="$WORK_DIR/office_list.tsv"
: > "$OFFICE_LIST"

TOTAL_FILES=0
OFFICE_COUNT=0
while IFS= read -r -d '' f; do
    TOTAL_FILES=$((TOTAL_FILES + 1))
    # Extraire l'extension (minuscule) sans le point
    base="${f##*/}"
    ext="${base##*.}"
    # Ignorer les fichiers sans extension (ext == base)
    [ "$ext" = "$base" ] && continue
    ext_lower=$(echo "$ext" | tr '[:upper:]' '[:lower:]')
    # L'extension est-elle bureautique ?
    [ -z "${EXT_CATEGORY[$ext_lower]+x}" ] && continue
    cat="${EXT_CATEGORY[$ext_lower]}"
    MIME=$(file --mime-type -b "$f" 2>/dev/null || echo "")
    SIZE=$(stat -c%s "$f" 2>/dev/null || echo 0)
    MTIME=$(stat -c%Y "$f" 2>/dev/null || echo 0)
    MTIME_ISO=$(date -d "@$MTIME" +"%Y-%m-%dT%H:%M:%S" 2>/dev/null || echo "")
    REL="${f#$INVESTIGATION_DIR/}"
    echo -e "${f}\t${MIME}\t${SIZE}\t${MTIME_ISO}\t${REL}\t${ext_lower}\t${cat}" >> "$OFFICE_LIST"
    OFFICE_COUNT=$((OFFICE_COUNT + 1))
done < <(find "$INVESTIGATION_DIR" -type f -print0 2>/dev/null)

echo -e "${GREEN}✅ ${OFFICE_COUNT} fichier(s) bureautique(s) détecté(s) sur ${TOTAL_FILES} fichier(s) total.${NC}"
if [ "$OFFICE_COUNT" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Aucun fichier bureautique trouvé.${NC}"
    log_action "success" "Aucun fichier bureautique trouvé"
    JSON_FILE="$WORK_DIR/resultat.json"
    DATE_ANALYSIS=$(date +"%d/%m/%Y à %H:%M:%S")
    cat > "$JSON_FILE" <<EOFJSON
{"date":"$DATE_ANALYSIS","total_files":$TOTAL_FILES,"office_count":0,"categories":{},"formats":{},"files":[]}
EOFJSON
    # --- Generation du rapport PDF (0 fichier) ---
    if [ "$GEN_PDF" -eq 1 ]; then
        PDF_SCRIPT="$(dirname "$0")/../pdf/vigil_pdf.py"
        if [ -f "$PDF_SCRIPT" ] && command -v python3 >/dev/null 2>&1; then
            mkdir -p "$RAPPORTS_DIR" 2>/dev/null || true
            PDF_OUT=$(python3 "$PDF_SCRIPT" --kind office --json "$JSON_FILE" \
                --user "$ACTIVE_USER" --project "$ACTIVE_PROJECT" --dir "$RAPPORTS_DIR" 2>&1)
            if [ $? -eq 0 ] && [ -n "$PDF_OUT" ] && [ -f "$PDF_OUT" ]; then
                echo -e "${GREEN}✅ Rapport PDF généré : $PDF_OUT${NC}"
                log_action "success" "Rapport PDF généré : $PDF_OUT"
                if [ -n "$PROJECT_LOG_DIR" ] && [ -d "$PROJECT_LOG_DIR" ] && [ -w "$PROJECT_LOG_DIR" ]; then
                    cp -f "$PDF_OUT" "$PROJECT_LOG_DIR/" 2>/dev/null && \
                        echo -e "${GREEN}   Rapport copié dans le dossier du projet${NC}"
                fi
            else
                echo -e "${RED}❌ Échec de la génération du rapport PDF.${NC}"
                ERRORS=$((ERRORS + 1))
            fi
        else
            echo -e "${YELLOW}⚠️  Génération PDF ignorée (python3 ou script PDF manquant).${NC}"
        fi
    fi
    # --- Copie du JSON pour rapport consolidé (mode multi) ---
    if [ -n "$JSON_OUT" ] && [ -f "$JSON_FILE" ]; then
        mkdir -p "$JSON_OUT" 2>/dev/null || true
        cp -f "$JSON_FILE" "$JSON_OUT/vigil_office.json" 2>/dev/null || true
    fi
    rm -f "$OFFICE_LIST" 2>/dev/null || true
    rm -rf "$WORK_DIR" 2>/dev/null || true
    final_pause
    exit 0
fi

echo ""

# --- Statistiques par categorie et par format ---
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  Étape 2 : Statistiques                     ${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

declare -A CAT_COUNT
declare -A EXT_COUNT
while IFS=$'\t' read -r path mime size mtime rel ext cat; do
    CAT_COUNT["$cat"]=$(( ${CAT_COUNT["$cat"]:-0} + 1 ))
    EXT_COUNT["$ext"]=$(( ${EXT_COUNT["$ext"]:-0} + 1 ))
done < "$OFFICE_LIST"

echo -e "${GREEN}✅ Répartition par catégorie :${NC}"
cat_labels_fr="document:Document spreadsheet:Tableur presentation:Présentation pdf:PDF rtf:RTF"
for cat in document spreadsheet presentation pdf rtf; do
    [ -n "${CAT_COUNT[$cat]:-}" ] || continue
    label_fr=$(echo "$cat_labels_fr" | tr ' ' '\n' | grep "^$cat:" | cut -d: -f2)
    [ -z "$label_fr" ] && label_fr="$cat"
    echo -e "  ${BOLD}${label_fr}${NC} ${GREY}:${NC} ${BOLD}${CAT_COUNT[$cat]}${NC} ${GREY}fichier(s)${NC}"
done
echo ""
echo -e "${GREEN}✅ Répartition par format :${NC}"
for ext in "${!EXT_COUNT[@]}"; do
    label="${EXT_LABEL[$ext]:-$ext}"
    echo -e "  ${BOLD}${label}${NC} ${GREY}:${NC} ${BOLD}${EXT_COUNT[$ext]}${NC} ${GREY}fichier(s)${NC}"
done
echo ""

# --- Copie optionnelle des fichiers bureautiques ---
if [ -n "$COPY_TO" ]; then
    echo -e "${BLUE}=== Copie des fichiers bureautiques ===${NC}"
    if [ ! -d "$COPY_TO" ]; then
        mkdir -p "$COPY_TO" 2>/dev/null || {
            echo -e "${RED}❌ Impossible de créer le dossier de destination : ${COPY_TO}${NC}"
            ERRORS=$((ERRORS + 1))
        }
    fi
    if [ -d "$COPY_TO" ]; then
        COPIED_OFFICE=0
        while IFS=$'\t' read -r path mime size mtime rel ext cat; do
            [ -z "$path" ] && continue
            [ -f "$path" ] || continue
            if cp -p "$path" "$COPY_TO/$(basename "$path")" 2>/dev/null; then
                COPIED_OFFICE=$((COPIED_OFFICE + 1))
            fi
        done < "$OFFICE_LIST"
        echo -e "${GREEN}✅ ${COPIED_OFFICE} fichier(s) bureautique(s) copié(s) vers : ${COPY_TO}${NC}"
    fi
    echo ""
fi

# --- Calcul des signatures SHA-256 (par fichier) ---
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  Étape 3 : Signatures SHA-256             ${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}Calcul des empreintes SHA-256 des fichiers...${NC}"
SHA_FILE="$WORK_DIR/sha256.tsv"
: > "$SHA_FILE"
SHA_OK=0
SHA_FAIL=0
while IFS=$'\t' read -r path mime size mtime rel ext cat; do
    [ -z "$path" ] && continue
    hash=$(sha256sum "$path" 2>/dev/null | awk '{print $1}')
    if [ -n "$hash" ]; then
        echo -e "${rel}\t${hash}" >> "$SHA_FILE"
        SHA_OK=$((SHA_OK + 1))
    else
        SHA_FAIL=$((SHA_FAIL + 1))
    fi
done < "$OFFICE_LIST"
echo -e "${GREEN}✅ ${SHA_OK} signature(s) calculée(s)${NC} ${GREY}(${SHA_FAIL} échec(s))${NC}"
echo ""

# --- Generation du JSON pour le PDF ---
JSON_FILE="$WORK_DIR/resultat.json"
DATE_ANALYSIS=$(date +"%d/%m/%Y à %H:%M:%S")

# Construction du JSON : stats par categorie + par format + tableau files
# via Python pour un echappement JSON correct (chemins/special chars).
if command -v python3 >/dev/null 2>&1; then
    python3 - "$OFFICE_LIST" "$SHA_FILE" "$JSON_FILE" "$DATE_ANALYSIS" \
           "$TOTAL_FILES" "$OFFICE_COUNT" <<'PYJSON'
import json, sys
office_list, sha_file, out, date, total, count = sys.argv[1:7]
# Index sha256 par chemin relatif de partition
sha = {}
try:
    with open(sha_file, encoding="utf-8") as fh:
        for line in fh:
            line = line.rstrip("\n")
            if not line:
                continue
            parts = line.split("\t", 1)
            if len(parts) == 2:
                sha[parts[0]] = parts[1]
except OSError:
    pass
# Comptage par categorie et par format + tableau files
categories = {}
formats = {}
files = []
try:
    with open(office_list, encoding="utf-8") as fh:
        for line in fh:
            line = line.rstrip("\n")
            if not line:
                continue
            cols = line.split("\t")
            if len(cols) < 7:
                continue
            path_abs, mime, size, mtime, rel, ext, cat = cols[:7]
            categories[cat] = categories.get(cat, 0) + 1
            formats[ext] = formats.get(ext, 0) + 1
            try:
                size_i = int(size)
            except ValueError:
                size_i = 0
            files.append({
                "path": rel,
                "sha256": sha.get(rel, ""),
                "size": size_i,
                "mtime": mtime,
                "mime": mime,
                "ext": ext,
                "category": cat,
            })
except OSError:
    pass
result = {
    "date": date, "total_files": int(total), "office_count": int(count),
    "categories": categories, "formats": formats, "files": files,
}
with open(out, "w", encoding="utf-8") as fh:
    json.dump(result, fh, ensure_ascii=False, indent=2)
PYJSON
else
    # Fallback sans python3 : stats seulement, pas de tableau files
    CAT_JSON=$(for c in "${!CAT_COUNT[@]}"; do printf '"%s":%s,' "$c" "${CAT_COUNT[$c]}"; done | sed 's/,$//')
    EXT_JSON=$(for e in "${!EXT_COUNT[@]}"; do printf '"%s":%s,' "$e" "${EXT_COUNT[$e]}"; done | sed 's/,$//')
    cat > "$JSON_FILE" <<EOFJSON
{"date":"$DATE_ANALYSIS","total_files":$TOTAL_FILES,"office_count":$OFFICE_COUNT,"categories":{$CAT_JSON},"formats":{$EXT_JSON},"files":[]}
EOFJSON
fi

# --- Generation du rapport PDF ---
echo -e "${BLUE}=== Génération du rapport PDF ===${NC}"
if [ "$GEN_PDF" -eq 0 ]; then
    echo -e "${YELLOW}⏹️  Génération du rapport PDF désactivée (--no-pdf).${NC}"
else
PDF_SCRIPT="$(dirname "$0")/../pdf/vigil_pdf.py"
if [ -f "$JSON_FILE" ] && [ -f "$PDF_SCRIPT" ] && command -v python3 >/dev/null 2>&1; then
    mkdir -p "$RAPPORTS_DIR" 2>/dev/null || true
    PDF_OUT=$(python3 "$PDF_SCRIPT" \
        --kind office \
        --json "$JSON_FILE" \
        --user "$ACTIVE_USER" \
        --project "$ACTIVE_PROJECT" \
        --dir "$RAPPORTS_DIR" 2>&1)
    if [ $? -eq 0 ] && [ -n "$PDF_OUT" ] && [ -f "$PDF_OUT" ]; then
        echo -e "${GREEN}✅ Rapport PDF généré : $PDF_OUT${NC}"
        log_action "success" "Rapport PDF généré : $PDF_OUT"
        # Copie du rapport PDF dans le dossier du projet (pour l'export).
        if [ -n "$PROJECT_LOG_DIR" ] && [ -d "$PROJECT_LOG_DIR" ] && [ -w "$PROJECT_LOG_DIR" ]; then
            cp -f "$PDF_OUT" "$PROJECT_LOG_DIR/" 2>/dev/null && \
                echo -e "${GREEN}   Rapport copié dans le dossier du projet : ${PROJECT_LOG_DIR}/$(basename "$PDF_OUT")${NC}"
        fi
    else
        echo -e "${RED}❌ Échec de la génération du rapport PDF.${NC}"
        echo -e "${YELLOW}    $PDF_OUT${NC}"
        ERRORS=$((ERRORS + 1))
    fi
else
    echo -e "${YELLOW}⚠️  Génération PDF ignorée (python3, JSON ou script PDF manquant).${NC}"
fi
fi
echo ""

# --- Copie du JSON pour rapport consolidé (mode multi) ---
if [ -n "$JSON_OUT" ] && [ -f "$JSON_FILE" ]; then
    mkdir -p "$JSON_OUT" 2>/dev/null || true
    cp -f "$JSON_FILE" "$JSON_OUT/vigil_office.json" 2>/dev/null || true
fi

# --- Nettoyage du dossier temporaire ---
rm -rf "$WORK_DIR" 2>/dev/null || true
log_action "success" "${OFFICE_COUNT} fichiers bureautiques analysés"

final_pause
