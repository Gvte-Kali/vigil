...
#
# Inspire de scalpel/bin/audios : extraction et organisation des fichiers
# audio depuis les peripheriques montes dans /investigation/.
#
# Le script :
#   1. detecte tous les fichiers audio dans /investigation/ (via `file`) ;
#   2. les classe par type MIME (mpeg, ogg, x-wav, ...) pour les statistiques ;
#   3. extrait la duree et le debit (bitrate) de chaque fichier via ffprobe
#      (equivalent des metadonnees pour l'audio) ;
#   4. classe les fichiers par duree (courte / moyenne / longue / sans) ;
#   5. calcule la signature SHA-256 de chaque fichier ;
#   6. genere un rapport PDF (dans /opt/vigil/rapports).
#
...
# --copy-to DIR  : copier les fichiers audio vers DIR
GEN_PDF=1
COPY_TO=""
JSON_OUT=""
while [ $# -gt 0 ]; do
...
WORK_DIR=$(mktemp -d -t "vigil_audio_${STAMP}.XXXXXX")
...
log_action() {
    ...
    local log_entry="[$timestamp] User:$ACTIVE_USER | Entity:$ACTIVE_ENTITY | Project:$ACTIVE_PROJECT | Action:audio_analysis | Status:$1 | Message:$2"
...
# Outil optionnel : ffprobe (ffmpeg) pour duree + debit
FFPROBE=""
if command -v ffprobe >/dev/null 2>&1; then
    FFPROBE="ffprobe"
    echo -e "${GREEN}✅ ffprobe détecté : extraction durée/débit disponible.${NC}"
else
    echo -e "${YELLOW}⚠️  ffprobe absent : extraction durée/débit désactivée.${NC}"
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  Vigil - Analyse des fichiers audio        ${NC}"
echo -e "${BLUE}============================================${NC}"
...
# --- Detection des fichiers audio ---
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  Étape 1 : Détection des fichiers audio   ${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${YELLOW}Recherche des fichiers audio dans /investigation/...${NC}"
echo ""

# Liste des fichiers audio via `file` (mime type audio/*)
# Format TSV : chemin_absolu<TAB>mime<TAB>taille<TAB>date_modif_iso<TAB>chemin_partition
AUDIO_LIST="$WORK_DIR/audio_list.tsv"
: > "$AUDIO_LIST"
...
while IFS= read -r -d '' f; do
    ...
    case "$MIME" in
        audio/*)
            ...
            # partition (1er sous-dossier) comme racine, ex: sdb1/dossier/son.mp3
            REL="${f#$INVESTIGATION_DIR/}"
            echo -e "${f}\t${MIME}\t${SIZE}\t${MTIME_ISO}\t${REL}" >> "$AUDIO_LIST"
            AUDIO_COUNT=$((AUDIO_COUNT + 1))
            ;;
    esac
done < <(find "$INVESTIGATION_DIR" -type f -print0 2>/dev/null)

echo -e "${GREEN}✅ ${AUDIO_COUNT} fichier(s) audio détecté(s) sur ${TOTAL_FILES} fichier(s) total.${NC}"
if [ "$AUDIO_COUNT" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Aucun fichier audio trouvé.${NC}"
    log_action "success" "Aucun fichier audio trouvé"
...
{"date":"$DATE_ANALYSIS","total_files":$TOTAL_FILES,"audio_count":0,"types":{},"duration_ok":0,"duration_fail":0,"ffprobe_available":$FFPROBE_AVAIL,"durations":{},"files":[]}
EOFJSON
    ...
    if [ "$GEN_PDF" -eq 1 ]; then
        ...
        if [ -f "$PDF_SCRIPT" ] && command -v python3 >/dev/null 2>&1; then
            ...
            PDF_OUT=$(python3 "$PDF_SCRIPT" --kind audio --json "$JSON_FILE" \
    ...
    if [ -n "$JSON_OUT" ] && [ -f "$JSON_FILE" ]; then
        ...
        cp -f "$JSON_FILE" "$JSON_OUT/vigil_audio.json" 2>/dev/null || true
    fi
    rm -f "$AUDIO_LIST" 2>/dev/null || true
...
# --- Copie optionnelle des fichiers audio ---
if [ -n "$COPY_TO" ]; then
    echo -e "${BLUE}=== Copie des fichiers audio ===${NC}"
    if [ ! -d "$COPY_TO" ]; then
    ...
    if [ -d "$COPY_TO" ]; then
        ...
        while IFS=$'\t' read -r path mime size mtime rel; do
            ...
            if cp -p "$path" "$COPY_TO/$(basename "$path")" 2>/dev/null; then
                COPIED_AUDIOS=$((COPIED_AUDIOS + 1))
            fi
        done < "$AUDIO_LIST"
        echo -e "${GREEN}✅ ${COPIED_AUDIOS} fichier(s) audio copié(s) vers : ${COPY_TO}${NC}"
    fi
    echo ""
fi

# --- Extraction duree + debit via ffprobe ---
DURATION_OK=0
DURATION_FAIL=0
DURATION_JSON="$WORK_DIR/durations.tsv"
: > "$DURATION_JSON"
# Repartition par duree : courte (<5min), moyenne (<20min), longue, sans
...
if [ -n "$FFPROBE" ]; then
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}  Étape 3 : Extraction durée & débit       ${NC}"
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}Extraction des métadonnées via ffprobe...${NC}"
    while IFS=$'\t' read -r path mime size mtime rel; do
        # duree (secondes) + debit (bit_rate en bits/s)
        dur=$(ffprobe -v quiet -of csv=p=0 -show_entries format=duration "$path" 2>/dev/null | tr -d '[:space:]')
        br=$(ffprobe -v quiet -of csv=p=0 -show_entries format=bit_rate "$path" 2>/dev/null | tr -d '[:space:]')
        if [ -z "$dur" ] || [ "$dur" = "N/A" ]; then
            ...
            dur_int=${dur%%.*}
            [ -z "$dur_int" ] && dur_int=0
            # Pour l'audio, les seuils sont plus courts (musique vs conference)
            if [ "$dur_int" -lt 300 ]; then bucket="courte"
            elif [ "$dur_int" -lt 1200 ]; then bucket="moyenne"
            else bucket="longue"
            fi
            DURATION_BUCKETS["$bucket"]=$(( ${DURATION_BUCKETS["$bucket"]} + 1 ))
            DURATION_OK=$((DURATION_OK + 1))
        fi
        echo -e "${path}\t${dur}\t${br}\t${bucket}" >> "$DURATION_JSON"
    done < "$AUDIO_LIST"
    ...
    DURATION_BUCKETS["sans"]=$AUDIO_COUNT
    DURATION_FAIL=$AUDIO_COUNT
fi
...
if command -v python3 >/dev/null 2>&1; then
    python3 - "$AUDIO_LIST" "$SHA_FILE" "$JSON_FILE" "$DATE_ANALYSIS" \
           "$TOTAL_FILES" "$AUDIO_COUNT" "$DURATION_OK" "$DURATION_FAIL" \
           "$FFPROBE_AVAIL" "$DUR_JSON" <<'PYJSON'
import json, sys
audio_list, sha_file, out, date, total, count, dur_ok, dur_fail, ffprobe, dur_json = sys.argv[1:11]
...
files = []
try:
    with open(audio_list, encoding="utf-8") as fh:
        for line in fh:
            line = line.rstrip("\n")
            if not line:
                continue
...
result = {
    "date": date, "total_files": int(total), "audio_count": int(count),
...
PYJSON
    ...
    cat > "$JSON_FILE" <<EOFJSON
{"date":"$DATE_ANALYSIS","total_files":$TOTAL_FILES,"audio_count":$AUDIO_COUNT,"types":{$TYPES_JSON},"duration_ok":$DURATION_OK,"duration_fail":$DURATION_FAIL,"ffprobe_available":$FFPROBE_AVAIL,"durations":$DUR_JSON,"files":[]}
EOFJSON
fi
...
rm -rf "$WORK_DIR" 2>/dev/null || true
log_action "success" "${AUDIO_COUNT} fichiers audio analysés"
